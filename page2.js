const PAGE2 = (() => {
  ...
  function renderInsurerChart(cases){
    ...
    const entries=Object.entries(byIns).filter(([,v])=>v.length>=3).map(([ins,hc])=>({ins:ins,val:Math.round(avg(hc.map(c=>c.approvalAmount)))})).sort((a,b)=>b.val-a.val).slice(0,15);
  ...
  function runComparison(){
    ...
    // Try strict filters first, then relax progressively to always get 3
    let recs=getRecommendations({city,insurer,tpa,category,procedure,topN:3});
    let relaxNote='';
    if(recs.length<3 && procedure){recs=getRecommendations({city,insurer,tpa,category,topN:3});relaxNote='Procedure filter relaxed';}
    if(recs.length<3 && tpa){recs=getRecommendations({city,insurer,category,topN:3});relaxNote='TPA filter relaxed';}
    if(recs.length<3 && insurer){recs=getRecommendations({city,category,topN:3});relaxNote='Insurer filter relaxed';}
    if(recs.length<3 && category){recs=getRecommendations({city,topN:3});relaxNote='Category filter relaxed';}
    if(recs.length<3){recs=getRecommendations({topN:3});relaxNote='All filters relaxed';}
    if(!recs.length){el.innerHTML='<div style="color:var(--text3);font-size:13px;padding:12px 0;">No hospitals found.</div>';return;}
    ...
    el.innerHTML=`<div style="font-size:12px;color:var(--text3);margin-bottom:12px;">Top 3 for: <strong style="color:var(--text);">${esc(desc)||'All combinations'}</strong>${relaxNote?` <span style="color:var(--amber);font-weight:600;">· ${relaxNote}</span>`:''}</div>
