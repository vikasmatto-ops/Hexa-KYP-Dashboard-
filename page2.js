const PAGE2 = (() => {
  ...
  const accOpen={Gold:true,Silver:false,Bronze:false,Underutilized:false};
  ...
  function renderTrend(cases){
    ...
    charts.trend=new Chart(ctx,{type:'line',data:{labels,datasets:[{label:'Avg ASP',data,borderColor:'#0ea5e9',backgroundColor:'rgba(14,165,233,.08)',borderWidth:2.5,pointRadius:4,pointBackgroundColor:'#0ea5e9',tension:.3,fill:true,datalabels:{display:true,color:'#0284c7',font:{size:10,weight:'700'},formatter:v=>'₹'+fmtN(v),anchor:'top',align:'top',offset:4}}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>`₹${fmtN(c.raw)}`,afterLabel:c=>`Cases: ${counts[c.dataIndex]}`}},datalabels:{display:false}},scales:{x:{ticks:{font:{size:10},maxRotation:35},grid:{display:false}},y:{ticks:{font:{size:10},callback:v=>'₹'+fmtN(v)},grid:{color:'#f0f0f0'}}}}});
  }
  ...
  function renderTopHospitals(){
    ...
    const combos={};
    cases.forEach(c=>{if(!c.approvalAmount)return;const key=`${c.hospitalName}||${c.category}||${c.insuranceName}`;if(!combos[key])combos[key]={hosp:c.hospitalName,city:c.city,cat:c.category,ins:c.insuranceName,tpa:c.tpaName,asp:[],count:0};combos[key].asp.push(c.approvalAmount);combos[key].count++;});
    ...
    el.innerHTML=`<table style="width:100%;border-collapse:collapse;font-size:13px;">
      <thead><tr style="background:var(--surface2);">
        <th style="padding:9px 12px;text-align:left;font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;border-bottom:1px solid var(--border);">#</th>
        <th style="padding:9px 12px;text-align:left;font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;border-bottom:1px solid var(--border);">Hospital</th>
        <th style="padding:9px 12px;text-align:left;font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;border-bottom:1px solid var(--border);">City</th>
        <th style="padding:9px 12px;text-align:left;font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;border-bottom:1px solid var(--border);">Category</th>
        <th style="padding:9px 12px;text-align:left;font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;border-bottom:1px solid var(--border);">Insurer</th>
        <th style="padding:9px 12px;text-align:left;font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;border-bottom:1px solid var(--border);">Avg ASP</th>
        <th style="padding:9px 12px;text-align:center;font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;border-bottom:1px solid var(--border);">Cases</th>
        <th style="padding:9px 12px;text-align:center;font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;border-bottom:1px solid var(--border);">Status</th>
      </tr></thead>
      <tbody>${top.map((r,i)=>{
        ...
        return`<tr style="border-bottom:1px solid var(--border);">
          ...
          <td style="padding:10px 12px;"><span class="rank-hosp-link" onclick="PAGE1.openPanel('${r.hosp.replace(/'/g,"\\'")}')">${esc(r.hosp.split(',')[0])}</span></td>
          <td style="padding:10px 12px;font-size:12px;color:var(--text2);">${cityLabel(r.city)}</td>
          <td style="padding:10px 12px;font-size:11px;font-weight:600;color:var(--text2);text-transform:uppercase;">${esc(r.cat)}</td>
          <td style="padding:10px 12px;font-size:12px;color:var(--text2);">${esc(r.ins)}${r.tpa&&r.tpa!=='In-House / Self'?`<div style="font-size:10px;color:var(--text3);">TPA: ${esc(r.tpa)}</div>`:''}</td>
          ...
          <td style="padding:10px 12px;text-align:center;font-size:13px;font-weight:700;color:var(--teal);">${r.count}</td>
  ...
  function renderInsurerChart(cases){
    ...
    charts.insurer=new Chart(ctx,{type:'bar',data:{labels:entries.map(e=>e.ins),datasets:[{label:'Avg ASP',data:entries.map(e=>e.val),backgroundColor:'#10b981',borderRadius:4}]},options:{...barOpts('₹',entries.map(e=>e.val)),indexAxis:'y'}});
  }

  function barOpts(px,vals){
    return{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>px+fmtN(c.raw)}},
    // Inline data labels
    },scales:{x:{ticks:{font:{size:10},maxRotation:35},grid:{display:false}},y:{ticks:{font:{size:10},callback:v=>px+fmtN(v)},grid:{color:'#f0f0f0'}}},
    // Chart.js built-in data labels via custom afterDraw
    animation:{onComplete:function(){
      const chart=this;const ctx2=chart.ctx;ctx2.save();ctx2.font='bold 10px DM Sans,sans-serif';ctx2.fillStyle='#374151';ctx2.textAlign='center';
      chart.data.datasets.forEach((dataset,i)=>{chart.getDatasetMeta(i).data.forEach((bar,j)=>{
        ...
        if(isHoriz){ctx2.textAlign='left';ctx2.fillText(lbl,bar.x+4,bar.y+4);}
        else{ctx2.fillText(lbl,bar.x,bar.y-5);}
      });});ctx2.restore();
    }}};
  }
  ...
  function runComparison(){
    ...
    if(!recs.length){el.innerHTML='<div style="color:var(--text3);font-size:13px;padding:12px 0;">No hospitals found. Try fewer filters.</div>';return;}
