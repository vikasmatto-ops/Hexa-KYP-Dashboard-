...
const PAGE2 = (() => {
  ...
  function populateComparators() {
    ...
    const opts=[['','Select hospital…'],...hosps.map(h=>[h,h])];
    fillSel('comp-a',opts,'');
    fillSel('comp-b',opts,'');
    const thYrs=[['','All Years'],...[...new Set(DATA.aspCases.map(c=>c.doaParsed&&c.doaParsed.getFullYear()).filter(Boolean))].sort().map(y=>[y,y])];
    const thMos=[['','All Months'],['01','Jan'],['02','Feb'],['03','Mar'],['04','Apr'],['05','May'],['06','Jun'],['07','Jul'],['08','Aug'],['09','Sep'],['10','Oct'],['11','Nov'],['12','Dec']];
    fillSel('th-year',thYrs,'');
    fillSel('th-month',thMos,'');
    fillSel('th-city',cities,'');
    fillSel('th-category',cats,'');
    fillSel('th-insurer',[['','All Insurers'],...getInsurers().map(i=>[i,i])],'');
    fillSel('th-tpa',[['','All TPAs'],...getTPAs().map(t=>[t,t])],'');
  }

  function onCompCityChange() {
    const city = document.getElementById('comp-city')?.value;
    const hosps = [...new Set(DATA.aspCases.filter(c=>!city||c.city===city).map(c=>c.hospitalName))].sort();
    const opts=[['','Select hospital…'],...hosps.map(h=>[h,h])];
    fillSel('comp-a',opts,''); fillSel('comp-b',opts,'');
  }
  ...
  function runComparison() {
    ...
    if(!a||!b||a===b){el.innerHTML='<div style="color:var(--text3);font-size:13px;">Select two different hospitals to compare.</div>';return;}
    const cat=document.getElementById('comp-cat')?.value;
    const result=compareHospitals(a,b,{category:cat});
    const {a:ha,b:hb,recommended}=result;
    el.innerHTML=`<div class="compare-grid">
      <div class="compare-col"><div class="compare-label">Hospital A</div><div class="compare-hosp-name">${esc(ha.hospitalName.split(',')[0])}</div>${compStats(ha)}</div>
      <div class="compare-vs">VS</div>
      <div class="compare-col"><div class="compare-label">Hospital B</div><div class="compare-hosp-name">${esc(hb.hospitalName.split(',')[0])}</div>${compStats(hb)}</div>
    </div>
    <div class="compare-winner">★ Recommended: ${esc(recommended.split(',')[0])}${ha.avgASP&&hb.avgASP?' — Higher ASP (₹'+fmtN(Math.max(ha.avgASP,hb.avgASP))+' vs ₹'+fmtN(Math.min(ha.avgASP,hb.avgASP))+')':''}</div>`;
  }

  function compStats(h) {
    return`<div class="compare-stat"><span class="compare-stat-label">Total cases</span><span class="compare-stat-value">${h.totalCases}</span></div>
    <div class="compare-stat"><span class="compare-stat-label">Avg ASP</span><span class="compare-stat-value">${h.avgASP?'₹'+fmtN(h.avgASP):'—'}</span></div>
    <div class="compare-stat"><span class="compare-stat-label">Business score</span><span class="compare-stat-value">${h.businessScore?'₹'+fmtN(h.businessScore):'—'}</span></div>
    <div class="compare-stat"><span class="compare-stat-label">Last case</span><span class="compare-stat-value">${h.lastCaseDate||'—'}</span></div>
    <div class="compare-stat"><span class="compare-stat-label">Top procedures</span><span class="compare-stat-value" style="font-size:11px;text-align:right;">${h.topProcedures.map(p=>`${esc(p.procedure)} (${p.count})`).join('<br>')}</span></div>`;
  }
  ...
  function bindEvents() {
    ...
    on('comp-city','change',onCompCityChange);
