const PAGE2 = (() => {
  ...
  const accOpen={Gold:true,Silver:true,Bronze:true,Underutilized:true};
  ...
  function runComparison(){
    ...
    // STRICT filtering — no relaxation. Active hospitals only.
    let recs=getRecommendations({city,insurer,tpa,category,procedure,topN:20});
    const activeNames=new Set(DATA.hospitals.filter(h=>h.activeStatus==='Active').map(h=>h.hospitalName.toLowerCase().trim()));
    recs=recs.filter(r=>activeNames.has(r.hospitalName.toLowerCase().trim())).slice(0,3);
    if(!recs.length){
