const PAGE1 = (() => {
  const F = {city:'',region:'',status:'',tier:'',insurer:'',tpa:'',search:''};
  let pg = 1; const PS = 20;
  ...
  function init() {
    ...
    renderTable();
    bindEvents();
    onDataRefresh(()=>{ renderRegionCards(); renderFilters(); renderTable(); });
  }

  // FIX: Active/Inactive counts per zone
  function renderRegionCards() {
    ...
    zones.forEach(z=>{
      const en=document.getElementById('rn-'+z);
      const es=document.getElementById('rs-'+z);
  ...
  function renderFilters() {
    ...
    // Smart finder recommendation filters
    fillSel('sf-city',    [['','All Cities'],   ...getCities().map(c=>[c,cityLabel(c)])], '');
    fillSel('sf-insurer', [['','Any Insurer'],  ...getInsurers().map(i=>[i,i])],          '');
    fillSel('sf-tpa',     [['','Any TPA'],      ...getTPAs().map(t=>[t,t])],             '');
    fillSel('sf-category',[['','Any Category'], ...CONFIG.ACTIVE_CATEGORIES.map(c=>[c,c])],'');
    fillSel('sf-procedure',[['','Any Procedure'],...getProceduresForCategory('').map(p=>[p,p])],'');
    // Distance finder filters
    fillSel('sf-ins-dist', [['','Any Insurer'],  ...getInsurers().map(i=>[i,i])], '');
    fillSel('sf-tpa-dist', [['','Any TPA'],      ...getTPAs().map(t=>[t,t])],    '');
  }
  ...
  function renderTable() {
    ...
    tbody.innerHTML=slice.map(h=>{
      ...
      const flagH=flags?`<span class="flag-badge" title="${flags} discrepancies">⚑${flags}</span>`:'';
  ...
  function renderPag(total) {
    ...
    let h='';
    if(pg>1) h+=`<button class="pg-btn" data-page="${pg-1}">‹</button>`;
  ...
  // FIX: Center modal detail panel with 2-column layout
  function openPanel(name) {
    ...
    const byYear={};
    hc.forEach(c=>{
      if(!c.doaParsed) return;
      const y=c.doaParsed.getFullYear();
      if(!byYear[y]) byYear[y]={cases:[],cats:{}};
      byYear[y].cases.push(c);
      if(c.category&&c.approvalAmount!==null){
        if(!byYear[y].cats[c.category]) byYear[y].cats[c.category]=[];
        byYear[y].cats[c.category].push(c.approvalAmount);
      }
    });
    ...
    // LEFT column: header + stats + comments + ASP history
    document.getElementById('detail-left').innerHTML=`
      <div style="margin-bottom:12px;">
        ...
        <div style="font-size:11px;color:var(--text3);margin-bottom:8px;"><span style="display:inline-flex;align-items:center;gap:4px;"><span style="width:7px;height:7px;border-radius:50%;background:${CONFIG.ZONE_COLORS?.[h.zone]||'#9ca3af'};display:inline-block;"></span>${h.zone}</span> • ${cityLabel(h.city)} • PIN ${h.pinCode} • ${h.mopStatus}</div>
        ${h.tier?`<span class="tier-badge tier-${h.tier.toLowerCase()}">${h.tier}</span>`:''}
      </div>
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:12px;">
        <div style="background:var(--surface2);border-radius:var(--r-sm);padding:8px;text-align:center;"><div style="font-size:9px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.4px;margin-bottom:2px;">Status</div><div style="font-size:12px;font-weight:700;color:var(--text);">${h.activeStatus}</div></div>
        <div style="background:var(--surface2);border-radius:var(--r-sm);padding:8px;text-align:center;"><div style="font-size:9px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.4px;margin-bottom:2px;">MOP Type</div><div style="font-size:12px;font-weight:700;color:var(--text);">${h.mopStatus||'—'}</div></div>
        <div style="background:var(--teal-lt);border-radius:var(--r-sm);padding:8px;text-align:center;"><div style="font-size:9px;font-weight:700;color:var(--teal-dk);text-transform:uppercase;letter-spacing:.4px;margin-bottom:2px;">Coverage</div><div style="font-size:12px;font-weight:700;color:var(--teal);">${cov}%</div></div>
      </div>
      <div style="margin-bottom:10px;">
        ...
        ${h.insComments?`<div style="background:var(--amber-lt);border-left:3px solid var(--amber);border-radius:0 6px 6px 0;padding:7px 10px;margin-bottom:5px;font-size:12px;color:#92400e;"><strong>Insurance:</strong> ${esc(h.insComments)}</div>`:'<div style="font-size:11px;color:var(--text3);padding:2px 0;">🟢 Insurance Comments — not yet added</div>'}
        ${h.cityComments?`<div style="background:var(--teal-lt);border-left:3px solid var(--teal);border-radius:0 6px 6px 0;padding:7px 10px;margin-bottom:5px;font-size:12px;color:var(--teal-dk);"><strong>City team:</strong> ${esc(h.cityComments)}</div>`:'<div style="font-size:11px;color:var(--text3);padding:2px 0;">🔵 City Comments — not yet added</div>'}
        ${h.doctorComments?`<div style="background:var(--green-lt);border-left:3px solid var(--green);border-radius:0 6px 6px 0;padding:7px 10px;font-size:12px;color:#065f46;"><strong>Doctors:</strong> ${esc(h.doctorComments)}</div>`:'<div style="font-size:11px;color:var(--text3);padding:2px 0;">🔵 Doctors Team Comments — not yet added</div>'}
      </div>
      ${hc.length?`
      <div style="font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px;">📊 ASP History <span style="font-style:italic;font-weight:400;color:var(--teal);">${esc(h.hospitalName.split(',')[0].toUpperCase())}</span></div>
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:10px;">
        ...
        <div style="text-align:center;padding:8px;background:var(--surface2);border-radius:var(--r-sm);"><div style="font-size:9px;color:var(--text3);text-transform:uppercase;font-weight:600;margin-bottom:2px;">Categories</div><div style="font-size:22px;font-weight:800;color:var(--text);">${new Set(hc.map(c=>c.category).filter(Boolean)).size}</div></div>
      </div>
      ${Object.entries(byYear).sort(([a],[b])=>b-a).map(([yr,d])=>{
        ...
        return`<div style="margin-bottom:8px;"><div style="font-weight:700;font-size:12px;margin-bottom:5px;">${yr} <span style="font-weight:400;font-size:11px;color:var(--text3);">${d.cases.length} cases • Avg ${yAvg?'₹'+fmtN(yAvg):'—'}</span></div>
        ${catE.map(([cat,vals])=>`<div style="display:flex;justify-content:space-between;padding:3px 0;border-bottom:1px solid var(--border);font-size:11px;"><span style="color:var(--text2);text-transform:uppercase;font-weight:500;">${esc(cat)}</span><span style="display:flex;gap:8px;"><span style="color:var(--text3);">${vals.length} cases</span><span style="color:var(--teal);font-weight:700;font-family:var(--mono);">₹${fmtN(Math.round(vals.reduce((s,v)=>s+v,0)/vals.length))}</span></span></div>`).join('')}
        </div>`;}).join('')}
      ...
      `:'<div style="font-size:12px;color:var(--text3);padding:8px 0;">No ASP cases found for this hospital.</div>'}`;

    // RIGHT column: TPA + Insurer empanelment grids
  ...
  function closePanel(){document.getElementById('detail-overlay').classList.remove('open');document.querySelectorAll('.hospital-row.selected').forEach(r=>r.classList.remove('selected'));}
  window.closeModal=closePanel;

  // Smart recommendations with category + procedure
  function runRecommendations() {
    const city     =document.getElementById('sf-city')?.value;
    const insurer  =document.getElementById('sf-insurer')?.value;
    const tpa      =document.getElementById('sf-tpa')?.value;
    const category =document.getElementById('sf-category')?.value;
    const procedure=document.getElementById('sf-procedure')?.value;
    const el=document.getElementById('sf-rec-results'); if(!el) return;

    if(!city&&!insurer&&!tpa&&!category&&!procedure){el.innerHTML='<div style="color:var(--text3);font-size:13px;">Select at least one filter to get recommendations.</div>';return;}

    const recs=getRecommendations({city,insurer,tpa,category,procedure,topN:8});
    if(!recs.length){el.innerHTML=`<div style="background:var(--amber-lt);border-radius:8px;padding:10px 14px;font-size:13px;color:var(--amber);">No historical cases found for this combination. Try removing some filters.</div>`;return;}

    const desc=[city?cityLabel(city):'',insurer||'',tpa||'',category||'',procedure||''].filter(Boolean).join(' · ');
    el.innerHTML=`<div style="font-size:13px;font-weight:700;margin-bottom:10px;color:var(--text);">⭐ Top ${recs.length} Hospitals — <span style="color:var(--text3);font-weight:400;">${esc(desc)}</span></div>
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:10px;">
      ${recs.map((r,i)=>{
        const medal=i===0?'🥇':i===1?'🥈':i===2?'🥉':`#${i+1}`;
        const hNet=DATA.hospitals.find(h=>h.hospitalName.toLowerCase().trim()===r.hospitalName.toLowerCase().trim());
        const isActive=hNet?.activeStatus==='Active';
        const isBest=i===0;
        return`<div style="background:${isBest?'linear-gradient(135deg,#0284c7,#0ea5e9)':'var(--surface2)'};border:1px solid ${isBest?'transparent':'var(--border)'};border-radius:10px;padding:14px;cursor:pointer;transition:transform .15s;" onclick="PAGE1.openPanel('${r.hospitalName.replace(/\\/g,'\\\\').replace(/'/g,"\\'")}')" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform=''">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap;">
            ...
            ${hNet?`<span style="font-size:11px;color:${isBest?'rgba(255,255,255,.75)':(isActive?'var(--green)':'var(--red)')};font-weight:600;">${hNet.activeStatus}</span>`:''}
          </div>
        </div>`;
      }).join('')}
    </div>`;
  ...
  function runFinder() {
    ...
    const ins=document.getElementById('sf-ins-dist')?.value;
    const tpa=document.getElementById('sf-tpa-dist')?.value;
  ...
  function bindEvents() {
    ...
    on('p1-clear','click',()=>{Object.keys(F).forEach(k=>F[k]='');pg=1;document.getElementById('p1-search').value='';renderFilters();renderTable();});
    ...
    on('sf-category','change',()=>{
      const cat=document.getElementById('sf-category')?.value;
      fillSel('sf-procedure',[['','Any Procedure'],...getProceduresForCategory(cat).map(p=>[p,p])],'');
    });
    ['sf-city','sf-insurer','sf-tpa','sf-procedure'].forEach(id=>on(id,'change',()=>{}));
