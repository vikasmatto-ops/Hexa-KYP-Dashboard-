const PAGE1 = (() => {
  ...
  function runRecommendations(){
    ...
    // Try strict filters first, then relax to always show 3+
    let recs=getRecommendations({city,insurer,tpa,category,procedure,topN:8});
    let relaxNote='';
    if(recs.length<3 && procedure){const r2=getRecommendations({city,insurer,tpa,category,topN:8});if(r2.length>recs.length){recs=r2;relaxNote='Procedure filter relaxed';}}
    if(recs.length<3 && tpa){const r2=getRecommendations({city,insurer,category,topN:8});if(r2.length>recs.length){recs=r2;relaxNote='TPA filter relaxed';}}
    if(recs.length<3 && insurer){const r2=getRecommendations({city,category,topN:8});if(r2.length>recs.length){recs=r2;relaxNote='Insurer filter relaxed';}}
    if(recs.length<3 && category){const r2=getRecommendations({city,topN:8});if(r2.length>recs.length){recs=r2;relaxNote='Category filter relaxed';}}
    if(!recs.length){el.innerHTML=`<div style="background:var(--amber-lt);border-radius:8px;padding:10px 14px;font-size:13px;color:var(--amber);">No historical cases found.</div>`;return;}
    const desc=[city?cityLabel(city):'',insurer||'',tpa||'',category||'',procedure||''].filter(Boolean).join(' · ');
    el.innerHTML=`<div style="font-size:13px;font-weight:700;margin-bottom:10px;">⭐ Top ${recs.length} Hospitals — <span style="color:var(--text3);font-weight:400;">${esc(desc)}</span>${relaxNote?` <span style="color:var(--amber);font-weight:600;">· ${relaxNote}</span>`:''}</div>
