...
function parseHospitalNetwork(rows) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    DATA.tpaNames.forEach((name, idx) => {
      tpaMap[name] = (r[CONFIG.TPA_COL_START + idx] || '').trim().toLowerCase() === 'yes';
    });

    const insurerMap = {};
    DATA.insurerNames.forEach((name, idx) => {
      insurerMap[name] = (r[CONFIG.INSURER_COL_START + idx] || '').trim().toLowerCase() === 'yes';
    });
...
function crossCheckEmpanelment() {
  ...
  DATA.hospitals.forEach(h => {
    ...
    DATA.insurerNames.forEach(ins => {
      if (!h.insurer[ins]) {
        // Fuzzy match: check if any case has matching insurer name (case-insensitive, trim)
        const insLower = ins.toLowerCase().trim();
        const found = hc.some(c => {
          const cIns = c.insuranceName.toLowerCase().trim();
          return cIns === insLower ||
                 // Handle slight variations: "New India Assurance Co. Ltd." vs "The New India Assurance Company Ltd."
                 (insLower.includes('new india') && cIns.includes('new india')) ||
                 (insLower.includes('icici lombard') && cIns.includes('icici lombard')) ||
                 (insLower.includes('icici prudential') && cIns.includes('icici prudential')) ||
                 (insLower.includes('care health') && cIns.includes('care health')) ||
                 (insLower.includes('star health') && cIns.includes('star health'));
        });
        if (found) {
          h.insurer[ins] = true;                  // AUTO-FILL as Yes
          h.empanelmentFlags[ins] = 'insurer';    // mark as auto-filled
        }
      }
    });
      ...
      if (!h.tpa[tpa]) {
        ...
        const found = hc.some(c => {
          ...
          return cTpa === tpaLower ||
                 (tpaLower.split(' ')[0].length > 3 && cTpa.includes(tpaLower.split(' ')[0]));
        });
        if (found) {
          h.tpa[tpa] = true;                       // AUTO-FILL as Yes
