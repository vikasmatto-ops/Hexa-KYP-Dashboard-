...
function parseHospitalNetwork(rows) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    hospitals.push({
      ...
      cityRaw,
      area:           (r[1] || '').trim(),
      pinCode:        String(r[2] || '').trim().replace('.0', ''),
...
function crossCheckEmpanelment() {
  ...
  DATA.hospitals.forEach(h => {
    ...
    DATA.insurerNames.forEach(ins => {
      if (!h.insurer[ins]) {
        const found = hc.some(c => c.insuranceName.toLowerCase().trim() === ins.toLowerCase().trim());
        if (found) h.empanelmentFlags[ins] = 'insurer';
      }
    });
    DATA.tpaNames.forEach(tpa => {
      if (!h.tpa[tpa]) {
        const found = hc.some(c => c.tpaName.toLowerCase().trim() === tpa.toLowerCase().trim());
        if (found) h.empanelmentFlags[tpa] = 'tpa';
      }
    });
  });
}
