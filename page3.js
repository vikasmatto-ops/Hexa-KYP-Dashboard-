const PAGE3 = (() => {
  ...
  function init(){renderCityPills();renderFilters();bindEvents();onDataRefresh(()=>{renderCityPills();renderFilters();if(mapInited){renderMarkers();renderGaps();updateCounts();}});}

  function initMap(){
    if(mapInited){
      if(map){setTimeout(()=>{map.invalidateSize();renderMarkers();},200);}
      return;
    }
    if(typeof L==='undefined'){setTimeout(initMap,300);return;}
    const el=document.getElementById('map');
    if(!el){setTimeout(initMap,300);return;}
    // Ensure div has height before init
    el.style.height='500px';
    map=L.map('map',{zoomControl:true}).setView([20.5937,78.9629],5);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{attribution:'© OpenStreetMap',maxZoom:18}).addTo(map);
    mapInited=true;
    // Critical: invalidateSize after browser renders the now-visible div
    setTimeout(()=>{map.invalidateSize();renderMarkers();renderGaps();updateCounts();},300);
  }
  ...
  function renderCityPills(){
    ...
    el.querySelectorAll('.pill').forEach(btn=>{btn.addEventListener('click',()=>{f.city=f.city===btn.dataset.city?'':btn.dataset.city;renderCityPills();if(mapInited){renderMarkers();renderGaps();updateCounts();}});});
  }
  ...
  function renderMarkers(){
    ...
    hosps.forEach(h=>{
      ...
      m.bindPopup(buildPopup(h),{maxWidth:280});
      m.addTo(map);markers.push(m);plotted++;

      // Pulsing gap circle for active+not empanelled
      if(h.activeStatus==='Active'&&(f.insurer||f.tpa)){
        ...
        if(!iE||!tE){
          const c=L.circle([jLat,jLng],{radius:10000,color:'#94a3b8',weight:1.5,dashArray:'6 4',fillColor:'transparent',opacity:.5});
          c.addTo(map);gapCircles.push(c);
        }
      }
    });
  ...
  function getColor(h){
    ...
    if(isA&&isE) return '#10b981';
    if(isA&&!isE)return '#f97316';
    return '#ef4444';
  }
  ...
  function updateCounts(){
    ...
    if(el2)el2.textContent=hosps.length-active;
    if(el3)el3.textContent=hosps.length;
  }
